...
export default function PropertyCard({ property }: PropertyCardProps) {
  const { id, title, location, price, priceUnit, beds, baths, area, status, image } = property;
    ...
    <div className="bg-white rounded-lg shadow-md overflow-hidden transition-transform hover:shadow-lg hover:-translate-y-1">
      ...
      <div className="p-4">
        ...
        <div className="flex justify-between items-center mb-4">
          ...
          {status === 'available' && (
            <WhatsAppButton
              message={`Hello, I'm interested in the property: ${title} (ID: ${id})`}
              small
            />
          )}
        </div>
        ...
        <Link href={`/properties/${id}`} className="block mt-4 text-center bg-blue-600 text-white py-2 rounded-md hover:bg-blue-700 transition">
          View Details
        </Link>
      </div>
    </div>
